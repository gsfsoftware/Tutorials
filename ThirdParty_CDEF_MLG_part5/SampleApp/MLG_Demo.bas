...
' include this if using SQL tools
...
' exclude this if using sql tools
...
%IDC_lblSurname         = 1003
%IDC_txtSurname         = 1002
%IDC_lblForename        = 1005
%IDC_txtForename        = 1004
%IDC_btnSearch          = 1006
...
CALLBACK FUNCTION ShowdlgDemoGridProc()
   ...
   LOCAL lngChanges AS LONG       ' number of records on a grid that have been amended
   LOCAL lngRecordsFailedValidation AS LONG ' number of records that havent saved
   '
   LOCAL o_lngSheetSlot AS LONG      ' output of sheet number currently selected
   LOCAL strSurname AS STRING        ' value of the Surname search field
   LOCAL strForename AS STRING       ' value of the forename search field
   LOCAL strSearchCriteria AS STRING ' used to store the search criteria
   LOCAL lngCriteriaCount AS LONG    ' used to count criteria parameters
   LOCAL strWildcard AS STRING       ' flag for wildcard search i.e. case independant
   '
  SELECT CASE AS LONG CB.MSG
              ...
              ' sort column
                ...
                IF lngChanges > 0 THEN
                  MSGBOX "There are unsaved changes on this worksheet" & _
                                $CRLF & "Please save them before sorting", _
                                %MB_ICONWARNING OR %MB_TASKMODAL,"Unsaved changes"
                  EXIT FUNCTION
                ELSE
                  IF mycol <> funGetGridPositionOfPrimarycolumn(lngForm,lngGrid, _
                                                                lngTab,hGrid1) THEN
                    g_lngSorting = %TRUE
    ...
    CASE %WM_COMMAND
      ...
      SELECT CASE AS LONG CB.CTL
        ' /* Inserted by PB/Forms 10-02-2021 10:59:29
        CASE %IDC_btnSearch
          IF CB.CTLMSG = %BN_CLICKED OR CB.CTLMSG = 1 THEN
          '  first check there are no changes to the grid not yet saved
            lngChanges = funCountChanges(hGrid1)
            IF lngChanges > 0 THEN
              MSGBOX "There are unsaved changes on this worksheet" & _
                                $CRLF & "Please save them before searching", _
                                %MB_ICONWARNING OR %MB_TASKMODAL,"Unsaved changes"
              EXIT FUNCTION
            END IF
          END IF
          '
          ' search based on the query entered by user
          IF funGetSelectedTabName(hGrid1, o_lngSheetSlot) = "Data" THEN
            strSQL = "EXEC dbo.spr_GetUserlist"
            '
            ' get the surname to be searched
            PREFIX "CONTROL GET TEXT CB.HNDL,"
              %IDC_txtSurname TO strSurname
              %IDC_txtForename TO strForename
            END PREFIX
            '
            IF strSurname = "" AND strForename = "" THEN
            ' get everything
              funGenericPopulateGrid(strFormName , strGridName, _
                                     strTabName ,_
                                     strSQL, hGrid1 , %AMDB)
              '
            ELSE
            ' search on just the surname and/or forename
            ' strSearchCriteria in format of "FullName," & strSurname & ",1|"

            ' intialise wildcard flag i.e. fuzzy search
            ' 1 = case independant
            ' 0 = exact match
              strWildcard = "1" ' intialise wildcard flag i.e. fuzzy search
              strSearchCriteria = ""
              IF strSurname <> "" THEN
                strSearchCriteria = "Surname," & strSurname & "," & _
                                     strWildcard & "|"
              END IF
              '
              IF strForename <> "" THEN
                strSearchCriteria =  strSearchCriteria & _
                                    "FirstName," & strForename & "," & _
                                     strWildcard & "|"
              END IF
              '
              IF ISTRUE funGenericPopulateGridEx(strFormName, _
                                  strGridName, strTabName,_
                                  strSQL, hGrid1, %AMDB, _
                                  strSearchCriteria) THEN
                ' get the data and fill the grid
                '
              END IF
              '
              ' update label with records returned
              CONTROL SET TEXT CB.HNDL, %IDC_lblRecordsReturned, _
                                 "Records returned = " & _
                                 FORMAT$(funGetRowsInGrid(hGrid1))
              '
        ...
        ' /* Inserted by PB/Forms 10-02-2021 10:58:01
        CASE %IDC_txtSurname

        CASE %IDC_txtForename
        ' */
...
#PBFORMS BEGIN DIALOG %IDD_dlgDemoGrid->->
  ...
  CONTROL ADD TEXTBOX, hDlg, %IDC_txtSurname, "", 85, 39, 100, 13
  CONTROL ADD LABEL,   hDlg, %IDC_lblSurname, "Surname", 10, 40, 75, 13
  CONTROL SET COLOR    hDlg, %IDC_lblSurname, %BLUE, -1
  CONTROL ADD TEXTBOX, hDlg, %IDC_txtForename, "", 85, 58, 100, 13
  CONTROL ADD LABEL,   hDlg, %IDC_lblForename, "Forename", 10, 59, 75, 13
  CONTROL SET COLOR    hDlg, %IDC_lblForename, %BLUE, -1
  CONTROL ADD BUTTON,  hDlg, %IDC_btnSearch, "Search", 340, 45, 155, 15
#PBFORMS END DIALOG
'
