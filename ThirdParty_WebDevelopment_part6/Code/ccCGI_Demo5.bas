...
FUNCTION PBMAIN () AS LONG
  ...
  ' pick up each parameter
    ...
    'exit function
  ELSE
  ' no parameters received so display the index page
...
FUNCTION funCreateIndexPage_v1() AS LONG
  ...
  strTemplate = EXE.PATH$ & "Page_Templates\UserListReport.txt"
...
' return a form with the pages the user can select
  ...
  strHTML = strHTML & "<input type=" & $DQ & "submit" & $DQ & " " & _
            ...
            "value=" & $DQ & "Get Report on new page" & $DQ & "/><br>" & $CRLF

  strHTML = strHTML & "<p><input type=" & $DQ & "button" & $DQ & " " & _
            "id =" & $DQ & "btnGetReport" & $DQ & " " & _
            "onclick=" & $DQ & "DisplayReport()" & $DQ & " " & _
            "value=" & $DQ & "Display Report on this page" & $DQ & " /></p>"
            '
            ' finish the form
