...
'
%IDM_InsertBefore = 3020
%IDM_InsertAfter  = 3021
'
' add globals for Treeview menu
GLOBAL hMenu AS DWORD
GLOBAL hMenuEdit AS DWORD
GLOBAL hTMenu AS DWORD        ' Handle for the Treeview menu
...
CALLBACK FUNCTION ShowdlgTREEVIEWProc()
  ...
  LOCAL hItem     AS LONG          ' handle for newly inserted item
  '
  ...
  '
  LOCAL lngUnselectedImage AS LONG  ' position in imagelist for Unselected
  LOCAL lngSelectedImage AS LONG    ' position in imagelist for Selected
  LOCAL lngImageData AS LONG        ' image date in the TV get user
  '
  SELECT CASE AS LONG CB.MSG
    ...
    ' Initialization handler
      ...
      funCreateTreeViewPopupMenu(hTree)
    '
    CASE %WM_NCACTIVATE
    ...
    ' message header UDT
      ...
      SELECT CASE AS LONG @pNMHDR.code
        ...
        ' right click has been clicked
          SetFocus hTree
          funTrackPopup CB.HNDL, hTMenu , %TPM_LEFTALIGN
          EXIT FUNCTION
        '
    ...
    CASE %WM_COMMAND
      ...
      SELECT CASE AS LONG CB.CTL
        CASE %IDM_InsertBefore
          ...
          ' get the handle of the node clicked on
            TREEVIEW GET SELECT CB.HNDL, %IDC_tvBooks TO hNode
            ' now work out the image this node uses
            TREEVIEW GET USER CB.HNDL, %IDC_tvBooks,hNode TO lngImageData
            lngUnselectedImage = LO(INTEGER,lngImageData)
            lngSelectedImage   = HI(INTEGER,lngImageData)

          ' get the handle of the parent
            TREEVIEW GET PARENT CB.HNDL,%IDC_tvBooks, hNode TO hParent
            '
            ' get the handle of a previous sibling - if any
            TREEVIEW GET PREVIOUS CB.HNDL, %IDC_tvBooks, hNode TO hItem
            IF hItem >0 THEN
            ' insert after the previous sibling
              TREEVIEW INSERT ITEM CB.HNDL, %IDC_tvBooks, hParent, hItem , _
                     lngUnselectedImage, lngSelectedImage, _
                     "New Item" TO hItem
            '
            ELSE
            ' insert as the first child of the parent
              TREEVIEW INSERT ITEM CB.HNDL, %IDC_tvBooks, hParent, _
                     %TVI_FIRST  , _
                     lngUnselectedImage, lngSelectedImage, _
                     "New Item" TO hItem
            '
          ...
          ' get the handle of the node clicked on
            ...
            TREEVIEW GET PARENT CB.HNDL,%IDC_tvBooks, hNode TO hParent
            '
            ' add the new node after the first node selected
            TREEVIEW INSERT ITEM CB.HNDL, %IDC_tvBooks, hParent, hNode , _
                     lngUnselectedImage, lngSelectedImage, _
                     "New Item" TO hItem
          END IF
          '
...
' first reset the treeview
  ...
  lngImageData = MAK(LONG,%Books,%Books)
  '
  TREEVIEW INSERT ITEM hDlg, lng_tvBooks, hParent, _
                       hAfter, lngImageList, _
                       lngSelectImage, strText TO hItem
  '
  ' save the image data in the tree
  TREEVIEW SET USER hDlg, lng_tvBooks, hItem, lngImageData
  '
  ...
  '
  ' store the image data
  lngImageData = MAK(LONG,%Folder,%Folder)
  '
  TREEVIEW INSERT ITEM hDlg, lng_tvBooks, hParent, _
                       hAfter, lngImageList, _
                       lngSelectImage, strText TO hItem
  ...
  ' found the node
    ...
    SELECT CASE strNodeRoot
      CASE "Fiction"
        ...
        lngImageData = MAK(LONG,%Fiction,%Fiction)
      CASE "Non-fiction"
        lngImageList   = %NonFiction
        lngSelectImage = %selNonFiction
        lngImageData = MAK(LONG,%NonFiction,%selNonFiction)
      CASE ELSE
        lngImageList   = 0
        lngSelectImage = 0
    END SELECT
...
END FUNCTION
'
FUNCTION funCreateTreeViewPopupMenu(hTree AS DWORD) AS LONG
' create the menu for the treeview
  MENU NEW POPUP TO hTMenu
  MENU ADD STRING,hTMenu, "Insert node before",%IDM_InsertBefore, %MF_ENABLED
  MENU ADD STRING,hTMenu, "Insert node after",%IDM_InsertAfter, %MF_ENABLED
  MENU ATTACH hTMenu, hTree
END FUNCTION
'
FUNCTION funTrackPopup( BYVAL hDlg AS LONG, _
                        BYVAL hSubMenu AS LONG, _
  ...
  ' first get the current location of the mouse
  GetCursorPos PA
  hWnd = WindowFromPoint(PA)
  '
  IF hDlg THEN
  ' initialise menu
    SendMessage hDlg, %WM_INITMENU, hSubMenu, BYVAL 0&
  END IF
  '
  Result = TrackPopupMenuEx( hSubMenu, %TPM_RETURNCMD OR _
                            %TPM_NONOTIFY OR nFlags, _
                             PA.X, PA.Y, hWnd, BYVAL 0& )
  IF Result <> 0 AND hDlg <> 0 THEN
    SendMessage hDlg, %WM_COMMAND, MAKLNG( Result, 0 ), BYVAL 0&
  END IF
  '
  FUNCTION = Result
  '
