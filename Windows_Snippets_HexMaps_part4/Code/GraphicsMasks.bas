...
%MaxTroopers  = 6   ' total number of troopers
%MaxDefenders = 6   ' total number of defenders
'
...
END ENUM
'
ENUM Side           ' Defender or Attacker
  Defender = 1
  Attacker
END ENUM
'
TYPE udtTroops
  ...
  y AS LONG         ' grid location
  z AS LONG         ' offset 1-6
  state AS LONG     ' troops current condition
  type AS LONG      ' type of trooper
END TYPE
'
...
' redraw the map
  ...
  '
  ' set the buildings
  funSetBuildings(hDlg,%FALSE)
  funSetTroops(hDlg,%FALSE)
  '
  GRAPHIC REDRAW
  '
END FUNCTION
'
FUNCTION funSetTroops(hDlg AS DWORD, lngRedo AS LONG) AS LONG
' set the troops location
  LOCAL lngR AS LONG
  LOCAL lngC AS LONG
  LOCAL lngZ AS LONG
  LOCAL lngT AS LONG
  ...
  LOCAL lngFound AS LONG
  '
  strBitmapMask = EXE.PATH$ & "\Graphics\BlueTrooper_1_mask.bmp"
  strBitmap = EXE.PATH$ & "\Graphics\BlueTrooper_1.bmp"
  '
  ...
  GRAPHIC BITMAP LOAD strBitmap, nWidth&, nHeight& TO hBmp
  '
  FOR lngT = 1 TO %MaxTroopers
    IF ISTRUE lngRedo THEN
      ' save the troopers positions as its first move
      ...
      IF lngT = 1 THEN
        lngZ = 1
        '
        REDIM g_uTroops(1 TO %MaxTroopers)
      ELSE
        INCR lngZ
      END IF
      '
      PREFIX "g_uTroops(lngT)."
        x = lngR
        y = lngC
        z = lngZ
      ...
      END PREFIX
    ELSE
      ' use the stored locations of troops
      lngR = g_uTroops(lngT).x
      lngC = g_uTroops(lngT).y
      lngZ = g_uTroops(lngT).z
      '
    END IF
    '
    SELECT CASE lngZ
