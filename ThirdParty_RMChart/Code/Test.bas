...
SUB DoTheChart(BYVAL hParentDlg AS LONG)
    ...
    tChart.nWidth       = 600
    tChart.nHeight      = 450
    tChart.nBackColor   = %ColorTransparent
    tChart.nCtrlStyle   = %RMC_CTRLSTYLEIMAGE
    tChart.sBgImage     = "E:\PowerBasic\PBWin80\PBWin80\RMChartCurrent\seasky.jpg"
    tChart.sFontName    = "Times New Roman TUR"
    ...
    tCaption.sText  =     "Builds Per Week"
    tCaption.nBackColor = %ColorDefault
    tCaption.nTextColor = %ColorKhaki
    tCaption.nFontSize  = 20
    ...
    tLabelAxis.sText          = ""
    szTemp = "20/01/2019*27/01/2019*01/02/2019*08/02/2019*15/02/2019*21/02/2019*01/03/2019*08/03/2019"
    nRetVal = RMC_AddLabelAxisI(%ID_RMC1, 1, szTemp, tLabelAxis)
    IF nRetVal < 0 THEN GOTO IsError
    '************** Add legend to region 1 *******************************
    tLegend.nLegendAlign    = %RMC_LEGEND_TOP
    tLegend.nLegendBackColor= %ColorTransparent
    ...
    tBarSeries.nColor            = %ColorTransparent
    tBarSeries.nIsHorizontal     = %FALSE
    tBarSeries.nWhichDataAxis    = 1
    tBarSeries.nValueLabelOn     = %RMC_VLABEL_DEFAULT
    tBarSeries.nPointsPerColumn  = 1
    tBarSeries.nHatchMode        = %RMC_HATCHBRUSH_OFF
    '****** Read data values ******
    REDIM aData(7)
    aData(0) = 2 : aData(1) = 4 : aData(2) = 6 : aData(3) = 8 : aData(4) = 9
    aData(5) = 15 : aData(6) = 10 : aData(7) = 3
    nRetVal = RMC_AddBarSeriesI(%ID_RMC1, 1, aData(0), 8, tBarSeries)
    IF nRetVal < 0 THEN GOTO IsError
    '************** Add Region 2 *****************************
    tRegion.nLeft       = 5
    tRegion.nTop        = 240
    ...
    tCaption.sText  =     "Build Connector Runs per week"
    tCaption.nBackColor = %ColorDefault
    tCaption.nTextColor = %ColorKhaki
    tCaption.nFontSize  = 20
    tCaption.nIsBold    = %TRUE
    ...
    tGrid.nWidth         = 550
    tGrid.nHeight        = 150
    nRetVal = RMC_AddGridI(%ID_RMC1, 2, tGrid)
    IF nRetVal < 0 THEN GOTO IsError
    ...
    tDataAxis.sUnit           = ""
    tDataAxis.sText           = ""
    nRetVal = RMC_AddDataAxisI(%ID_RMC1, 2, tDataAxis)
    IF nRetVal < 0 THEN GOTO IsError
    ...
    tLabelAxis.nAlignment     = %RMC_LABELAXISBOTTOM
    tLabelAxis.nFontsize      = 8
    tLabelAxis.nTextColor     = %ColorDeepYellow
    tLabelAxis.nTextAlignment = %RMC_TEXTCENTER
    tLabelAxis.nLineColor     = %ColorDefault
    tLabelAxis.nLineStyle     = %RMC_LINESTYLESOLID
    tLabelAxis.sText          = ""
    szTemp = "20/01/2019*27/01/2019*01/02/2019*08/02/2019*15/02/2019*21/02/2019*01/03/2019*08/03/2019"
    nRetVal = RMC_AddLabelAxisI(%ID_RMC1, 2, szTemp, tLabelAxis)
    IF nRetVal < 0 THEN GOTO IsError
    '************** Add Series 1 to region 2 *******************************
    tBarSeries.nType             = %RMC_BARSINGLE
    tBarSeries.nStyle            = %RMC_COLUMN_3D_GRADIENT
    tBarSeries.nIsLucent         = %TRUE
    tBarSeries.nColor            = %ColorTransparent
    tBarSeries.nIsHorizontal     = %FALSE
    tBarSeries.nWhichDataAxis    = 1
    ...
    '****** Read data values ******
    REDIM aData(7)
    aData(0) = 1 : aData(1) = 2 : aData(2) = 3 : aData(3) = 4 : aData(4) = 5
    aData(5) = 6 : aData(6) = 7 : aData(7) = 8
    nRetVal = RMC_AddBarSeriesI(%ID_RMC1, 2, aData(0), 8, tBarSeries)
    IF nRetVal < 0 THEN GOTO IsError
