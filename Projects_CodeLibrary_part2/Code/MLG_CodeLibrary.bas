...
%IDD_dlgMLG_lite = 101
%IDABORT         =   3
%IDC_lblSearchFor = 1001
%IDC_txtSearchFor = 1002
%IDC_SEARCH       = 1003
%IDC_lblFound     = 1004
...
'
  ...
  '
  LOCAL strFoldername AS STRINGZ * %MAX_PATH ' for dragged folder
  LOCAL lngI AS LONG               ' count of files dragged
  '
  SELECT CASE AS LONG CB.MSG
    ...
    '
      funSetRowHeightForGrid(hGrid1,50)
      '
      ' Select sheet 2
      mSelectSheet(hGrid1,2)
      funSetRowHeightForGrid(hGrid1,50)
      funMarkGridasReadOnly(hGrid1)
      '
      ' reselect sheet 1
      mSelectSheet(hGrid1,1)
      funMarkGridasReadOnly(hGrid1)
      '
    CASE %WM_DROPFILES
    ' handle drop file event
      DragQueryFile CB.WPARAM,lngI,strFoldername, SIZEOF(strFoldername)
      DragFinish CB.WPARAM
      '
       ' now load the grid
      IF ISTRUE ISFOLDER(strFoldername) THEN
      ' only load if the item dragged is a folder
        ' select and clear the search grid sheet/tab
        mSelectSheet(hGrid1,2)
        funGridClear(hGrid1)
        '
        ' select sheet/tab 1 and update the library
        mSelectSheet(hGrid1,1)
        funLoadLibraries(strFoldername & "", hGrid1)
      END IF
    '
    CASE %WM_NOTIFY
    ...
    CASE %WM_COMMAND
      ...
      SELECT CASE AS LONG CB.CTL
        ...
        CASE %IDC_SEARCH
          IF CB.CTLMSG = %BN_CLICKED OR CB.CTLMSG = 1 THEN
          ' search for data in the description column
            CONTROL GET TEXT CB.HNDL,%IDC_txtSearchFor TO strText
            IF TRIM$(strText) <> "" THEN
              mSelectSheet(hGrid1,1)
              funRunTheSearch(CB.HNDL,hGrid1,strText)
            END IF
            '
          END IF
          '
...
#PBFORMS BEGIN DIALOG %IDD_dlgMLG_lite->->
  ...
  DIALOG NEW hParent, "MLG Lite Code Library", 202, 260, 720, 428, %WS_POPUP OR %WS_BORDER _
  ...
  CONTROL ADD BUTTON, hDlg, %IDABORT, "Exit", 615, 395, 50, 15
  CONTROL ADD LABEL,   hDlg, %IDC_lblSearchFor, "Search for", 15, 45, 60, 10
  CONTROL SET COLOR    hDlg, %IDC_lblSearchFor, %BLUE, -1
  CONTROL ADD TEXTBOX, hDlg, %IDC_txtSearchFor, "", 85, 40, 175, 16
  CONTROL ADD BUTTON,  hDlg, %IDC_SEARCH, "Search", 265, 40, 50, 15, _
    ...
    %WS_EX_LTRREADING
  CONTROL ADD LABEL,   hDlg, %IDC_lblFound, "", 15, 60, 100, 10
#PBFORMS END DIALOG
'
' set to accept drag and drop
 ...
 ' add the icons and buttons to the blank toolbar
  ...
  mPrepGrid(hGrid1,%MaxGridRows,%MaxGridColumns," Library ", 1)
  '
  ...
  '
  ' now add the second search sheet
  funAddSheet(hGrid1,"Search Results")
  '
...
END FUNCTION
'
FUNCTION funRunTheSearch(hDlg AS DWORD, _
                         hGrid AS DWORD, _
                         strText AS STRING) AS LONG
' run the search on the description column looking for matches
  DIM a_strLibData() AS STRING ' data to be searched
  DIM a_strResults() AS STRING ' results found
  '
  LOCAL lngRows AS LONG      ' rows in grid
  LOCAL lngColumns AS LONG   ' columns in grid
  LOCAL lngFound AS LONG     ' results counter
  LOCAL lngRow AS LONG       ' row counter
  LOCAL lngColumn AS LONG    ' column counter
  LOCAL lngDescriptionColumn AS LONG ' the description column number
  '
  lngDescriptionColumn = 5
  '
  lngRows = funGetRowsInGrid(hGrid)
  lngColumns = funGetColumnsInGrid(hGrid)
  '
  REDIM a_strLibData(lngRows,lngColumns)
  ' populate the grid
  MLG_GetEx (hGrid,a_strLibData())
  '
  ' prep results grid
  REDIM a_strResults(lngRows,lngColumns)
  '
  strText = UCASE$(strText)
  '
  FOR lngRow = 1 TO lngRows
    IF INSTR(UCASE$(a_strLibData(lngRow, _
                    lngDescriptionColumn)), _
                    strText) > 0 THEN
      ' found a match
      INCR lngFound
      FOR lngColumn = 1 TO lngColumns
        a_strResults(lngFound,lngColumn) = _
                     a_strLibData(lngRow,lngColumn)
      NEXT lngColumn
      '
    END IF
  NEXT lngRow
  '
  mSelectSheet(hGrid,2)
  funGridClear(hGrid)
  ' put array into search results grid with refresh
  MLG_PutEx(hGrid,a_strResults(),-4, 1)
  '
  FOR lngRow = 1 TO lngFound
  ' do the word wrap
    SendMessage hGrid ,%MLG_SETFORMATOVERRIDEEX ,_
                MAKLNG(lngRow,lngDescriptionColumn), _
                MAKLNG(%MLG_TYPE_JUST,%MLG_JUST_WORDWRAP)
  NEXT lngRow
  '
  ...
  '
  CONTROL SET TEXT hDlg,%IDC_lblFound,FORMAT$(lngFound) & _
                   " matches found"
  '
END FUNCTION
