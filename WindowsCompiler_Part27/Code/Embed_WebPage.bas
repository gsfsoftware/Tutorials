...
'
%IDC_GRAPHIC1 = 1100    ' graphic control
'
GLOBAL hFont1 AS DWORD         ' global for font
...
#INCLUDE "..\Libraries\PB_Winapi.inc"
'------------------------------------------------------------------------------
'   ** Constants **
'------------------------------------------------------------------------------
...
' https://www.gsfsoftware.co.uk/PBTutorials/Projects.htm
    ...
    STATIC strHint AS STRING
    STATIC lngLocation AS LONG
    STATIC lngIteration AS LONG
    '
    SELECT CASE AS LONG CB.MSG
      CASE %WM_INITDIALOG
      ' Initialization handler
      '
        hTimer = setTimer(CB.HNDL, 1&, 50&, BYVAL %NULL)
        strHint = "Text the user really needs to see goes here"
      '
      ...
      ' handle the timer
        INCR lngLocation
        GRAPHIC SET POS (0,0)
        IF lngLocation > LEN(strHint) THEN
          INCR lngIteration
          IF lngIteration >= 60 THEN
            lngLocation = 1
            lngIteration = 0
            GRAPHIC CLEAR
          ELSE
        ...
        END IF
        '
        GRAPHIC PRINT "   " & MID$(strHint,1, lngLocation)
        GRAPHIC REDRAW
      '
      CASE %WM_EraseBKGND
      ' display a gradient background colour
        hDC = CB.WPARAM
        DrawGradient hDC, 0
        FUNCTION = 1
        EXIT FUNCTION
...
#PBFORMS BEGIN DIALOG %IDD_dlgReporter->->
  ...
  CONTROL SET COLOR     hDlg, %IDC_lblWidth, %BLUE, -2
  CONTROL ADD LABEL,    hDlg, %IDC_lblHeight, "Chart Height", 75, 50, 50, 10
  CONTROL SET COLOR     hDlg, %IDC_lblHeight, %BLUE, -2
  CONTROL ADD LABEL,    hDlg, %IDC_lblBackground, "Chart Background Colour", _
    20, 90, 100, 10
  CONTROL SET COLOR     hDlg, %IDC_lblBackground, %BLUE, -2
  ...
  CONTROL ADD CHECKBOX, hDlg, %IDC_chkGradient, "Gradient fill", 70, 103, 55, _
    11
  CONTROL SET COLOR     hDlg,%IDC_chkGradient,%BLUE,-2
  ...
  CONTROL ADD LABEL,    hDlg, %IDC_lblColumns, "How Many Columns are there to " + _
    "be?", 255, 90, 120, 10
  CONTROL SET COLOR     hDlg, %IDC_lblColumns, %BLUE, -2
#PBFORMS END DIALOG
  '
  CONTROL ADD GRAPHIC , hDlg, %IDC_Graphic1,"",0,1,685,13
  FONT NEW "Helvetica", 12,0,%ANSI_CHARSET TO hFont1
  GRAPHIC ATTACH hDlg, %IDC_Graphic1, REDRAW
  GRAPHIC CLEAR
  GRAPHIC SET FONT hFont1
  GRAPHIC COLOR %RED, %RGB_GAINSBORO
  '
  AttachMENU1 hDlg
