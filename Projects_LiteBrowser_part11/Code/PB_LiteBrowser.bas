...
'
TYPE udtImgTag      ' type for top right and bottom left coords
  lngTopX AS LONG   ' of an image
  lngTopY AS LONG
  lngBottomX AS LONG
  lngBottomY AS LONG
  strURL AS STRING * 1024
END TYPE
'
GLOBAL g_uURLs() AS udtImgTag       ' array for URL links
'
TYPE udtObject        ' type for start and end locations of objects
  lngStart AS POINT
  lngEnd AS POINT
END TYPE
'
...
FUNCTION PBMAIN()
  ...
  REDIM g_uURLs(%MAX_URLS)          ' for URL links
  '
...
END FUNCTION
'
FUNCTION funProcess_a_URL(strURL AS STRING) AS LONG
...
'--------------------------------------------------------------------
  ...
  SELECT CASE wMsg
    ...
    ' mouse has moved
      ...
      strURL = "" '
      IF ISTRUE funIsLink(lngX,lngY,strURL) THEN
...
'
  ...
  FOR lngR = 1 TO UBOUND(g_uURLs)
  ' sweep through all possible stored hyperlinks
    IF lngX > g_uURLs(lngR).lngTopX AND _
       lngX < g_uURLs(lngR).lngBottomX AND _
       lngY > g_uURLs(lngR).lngTopY AND _
       lngY < g_uURLs(lngR).lngBottomY THEN
    ' this is a link area
      strURL = TRIM$(g_uURLs(lngR).strURL)
      FUNCTION = %TRUE
      EXIT FUNCTION
    END IF
  NEXT lngR
  ...
  ' step through each character
    ...
    ' end of a tag
      ...
      SELECT CASE strCommand
        ...
        ' start of image tag
          ...
          ' strAttributes should hold any attributes
            ...
            ' advance the current URL
              INCR lngURL
              ' populate the link
              uImgTag.strURL = strURL
              ' save the udt into the global array
              g_uURLs(lngURL) = uImgTag
            '
            END IF
          '
          END IF
        ...
        ' start of a ref tag
          lngRefTag = %TRUE
          strURL = TRIM$(PARSE$(strAttributes,"=",2),$DQ)
          ' get the data
        ...
        CASE "table"
        ' handle tables
          funBuildTable(lngStart, strAttributes)
          '
        CASE ELSE
...
FUNCTION funBuildTable(lngStart AS LONG, _
                       strTableAttributes AS STRING) AS LONG
' build a table structure
  ...
  funDisplayTable(a_strTable(), strCaption, _
                  lngHeader, LCASE$(strTableAttributes))
  '
END FUNCTION
'
...
' display the table on the graphics control
  ...
  FOR lngRow = 1 TO lngRows
    ...
    END IF
    '
    FOR lngColumn = 1 TO UBOUND(a_strTable,2)
    ' print the column data
      ...
      END IF
      '
      GRAPHIC PRINT a_strTable(lngRow,lngColumn);
      '
    NEXT lngColumn
    '
    IF lngRow = 1 AND lngHeader = %TRUE THEN
  ...
  IF ISTRUE lngTableBorder THEN
    ...
    ' draw vertical lines
      PREFIX "uVerticalLines."
        lngStart.X = uTable.lngStart.X + a_lngTableColumns(lngColumn) - lng_imgW
    ...
    '
    ' reset position
    'GRAPHIC SET POS (uTempPosition.lngStart.X, _
    '                 uTempPosition.lngStart.Y)
    '
  END IF
  '
END FUNCTION
...
FUNCTION funWidthOfTable(BYREF a_strTable() AS STRING, _
                         lng_imgW AS LONG, _
                         lng_imgH AS LONG, _
                         BYREF a_lngTableColumns() AS LONG) AS LONG
...
'
  ...
  ' look through each column
    ...
    ' of that column
      a_lngTableColumns(lngColumn) = MAX(LEN(a_strTable(lngRow,lngColumn)), _
                                         LEN(a_strTable(lngRow -1,lngColumn)))
      ' store width plus 2 padding characters
      a_lngTableColumns(lngColumn) = (a_lngTableColumns(lngColumn) * lng_imgW) + _
                                     (2 * lng_imgW)
    NEXT lngRow
  NEXT lngC
  '
...
FUNCTION funDisplayImage(hDlg AS DWORD, _
                         ...
                         lngInParagraph AS LONG) AS LONG
...
END FUNCTION
'
FUNCTION funGetTableTagValue(lngStartPosition AS LONG, _
