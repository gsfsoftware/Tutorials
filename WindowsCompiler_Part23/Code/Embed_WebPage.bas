...
' build a local html report
  ...
  ' set the gradients
  strHTML = strHTML & funSetGradients()
...
'------------------------------------------------------------------------------
FUNCTION funSetGradients() AS STRING
' define the SVG gradients
  LOCAL strGradient AS STRING

  strGradient = "<linearGradient id=""grad1"" x1=""0%"" y1=""0%"" x2=""100%"" y2=""0%"">" & _
      "<stop offset=""0%"" style=""stop-color:rgb(210,210,210);stop-opacity:0.2"" />" & _
      "<stop offset=""100%"" style=""stop-color:rgb(10,10,10);stop-opacity:0.2"" />" & _
      "</linearGradient>"
  FUNCTION = "<defs>" & strGradient & "</defs>"
  '
END FUNCTION
'
