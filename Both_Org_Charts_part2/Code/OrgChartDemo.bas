...
' convert the HR file
  ...
  ' prepare the Output array
    ...
    FUNCTION = funArrayDump(strOutput,a_strOutput())
    '
  ELSE
         ...
         CASE ELSE
          ...
          SELECT CASE lngElements - lngPrevElements
            ...
            ' element count going down
              funAddToHTML(REPEAT$(lngPrevElements-lngElements,"</LI></UL>"))
              'funAddToHTML("</UL></LI>")
          END SELECT
          '
          strColour = " style=""background-color:" & _
                      funGetDeptColour(strDepartment) & ";"""
                      '
          funAddToHTML("<li><span" & strColour & ">" & strName & "</span>" )
          '
       END SELECT
     '
     NEXT lngS
...
END FUNCTION
'
FUNCTION funGetDeptColour(strDepartment AS STRING) AS STRING
