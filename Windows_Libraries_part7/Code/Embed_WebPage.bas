...
'------------------------------------------------------------------------------
%ID_Folder          = 1006      ' icon for browse button
%ID_OCX             = 8000      ' id of embedded web page
'------------------------------------------------------------------------------
'   ** Declarations **
'------------------------------------------------------------------------------
#PBFORMS DECLARATIONS
'------------------------------------------------------------------------------
#RESOURCE ICON, Folder, "..\Libraries\Graphics\Load.ico"
'------------------------------------------------------------------------------
      ...
      ' Process control notifications
        SELECT CASE AS LONG CB.CTL
          ...
          CASE %IDC_imgBrowse
            IF CB.CTLMSG = %BN_CLICKED OR CB.CTLMSG = 1 THEN
              lngFlags = 0
              DISPLAY OPENFILE CB.HNDL,100,50,"Select local html file", _
                      EXE.PATH$, CHR$("HTML", 0, "*.htm;*.html", 0),"","", _
                      lngFlags TO strURL
              IF strURL <> "" THEN
...
#PBFORMS BEGIN DIALOG %IDD_dlgReporter->->
  ...
  DIALOG NEW hParent, "HTML Reporter", 225, 137, 672, 405, %WS_POPUP OR _
  ...
  CONTROL ADD BUTTON,    hDlg, %IDOK, "Display the HTML page", 400, 25, 110, _
    15
  DIALOG  SEND           hDlg, %DM_SETDEFID, %IDOK, 0
  CONTROL ADD TEXTBOX,   hDlg, %IDC_txtURL, "", 15, 26, 330, 13
  CONTROL ADD LABEL,     hDlg, %IDC_lblURL, "Enter URL here ", 15, 16, 295, _
    10
  CONTROL SET COLOR      hDlg, %IDC_lblURL, %BLUE, -1
  CONTROL ADD IMGBUTTON, hDlg, %IDC_imgBrowse, "", 350, 25, 20, 18, %WS_CHILD _
    OR %WS_VISIBLE OR %WS_TABSTOP OR %BS_BITMAP OR %BS_PUSHLIKE OR _
    %BS_PUSHBUTTON OR %BS_CENTER OR %BS_VCENTER, %WS_EX_LEFT OR _
    %WS_EX_LTRREADING
#PBFORMS END DIALOG
  '
  CONTROL SET IMGBUTTON hDlg, %IDC_imgBrowse, "Folder"
  '
