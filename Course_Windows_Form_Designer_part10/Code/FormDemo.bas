...
%LV_Columns = 6                  ' number of columns in the staff listview grid
'
...
CALLBACK FUNCTION ShowTitleProc()
  ...
  SELECT CASE AS LONG CB.MSG
    ...
    ' Initialization handler
      ...
      funPopulateListView(CB.HNDL,%IDC_lvStaffDataGrid)
      '
    CASE %WM_NCACTIVATE
            ...
            ' User double-clicked the ListView
              ...
              IF lngClickedItem >= 0 THEN
               ' item has been clicked - starts count from Zero
                ' MSGBOX "Double-clicked item index: " & _
                '         FORMAT$(lngClickedItem)
                '
        ...
        CASE %IDC_Submit
          ...
          ' so validate input
            ...
            ' and confirm to user
              ...
              INCR lngRow ' advance to next row
              ' insert a new row
              LISTVIEW INSERT ITEM CB.HNDL, %IDC_lvStaffDataGrid, _
                 lngRow, 0, strTitle
              ' populate the remaining columns
              PREFIX "LISTVIEW SET TEXT CB.HNDL, %IDC_lvStaffDataGrid,lngRow, "
                %lvColumn.Name, strName
                %lvColumn.Department, strDepartment
                %lvColumn.StartDate, strStartDate
                %lvColumn.Age, strAge
                %lvColumn.WorkingPattern, strWorkingPattern
              END PREFIX
              '
              ' fit column widths to data in listview
              FOR lngColumn = 1 TO %LV_Columns
                LISTVIEW FIT HEADER CB.HNDL, %IDC_lvStaffDataGrid, _
                                             lngColumn
              NEXT lngColumn
              ' save the listview
...
#PBFORMS BEGIN DIALOG %IDD_dlgGetTitle->%IDR_MENU1->
  ...
  DIALOG NEW hParent, "Enter the Title", 279, 122, 653, 377, %WS_POPUP OR _
  ...
  CONTROL ADD LISTVIEW, hDlg, %IDC_lvStaffDataGrid, "Listview1", 240, 95, _
    175, 120, %WS_CHILD OR %WS_VISIBLE OR %LVS_REPORT OR %LVS_SHOWSELALWAYS, _
    %WS_EX_CLIENTEDGE OR %WS_EX_LEFT
  CONTROL ADD LABEL,    hDlg, %IDC_lblCurrentStaff, "Current Staff ", 240, _
    80, 165, 15
  ...
  ' only if the file exists
    ...
    ' populate the list view
      ...
      LISTVIEW INSERT ITEM hDlg, %IDC_lvStaffDataGrid, _
                                 lngRow, 0, a_strStaffData(lngRow,%lvColumn.Title)
      ' populate the remaining columns
