...
END FUNCTION
'
FUNCTION funHasParentControl(hDlg AS DWORD, _
                             lngConfigRow AS LONG, _
                             strData AS STRING) AS LONG
' look for other controls that have this as a child
  LOCAL lngR AS LONG
  LOCAL lngObject AS LONG
  LOCAL strObjectType AS STRING
  LOCAL lngParent AS LONG
  LOCAL strSelection AS STRING
  LOCAL strTrigger AS STRING
  LOCAL lngRefID AS LONG
  LOCAL lngValue AS LONG
  LOCAL lngLabelControl AS LONG
  '
  '
  ' get the ref of the parent
  lngRefID = VAL(PARSE$(ga_strConfig(lngConfigRow),"",%Ref_ID))
  lngParent = VAL(PARSE$(ga_strConfig(lngConfigRow),"",%Ref_Conditional))
  ' pick up any associated label control
  lngLabelControl = VAL(PARSE$(ga_strConfig(lngConfigRow),"", _
                    %Ref_LabelControl))
  ' get the trigger value config to check against the parent
  strTrigger = PARSE$(ga_strConfig(lngConfigRow),"",%Ref_Selection)
  ' exit if control has no parent
  IF lngParent = 0 THEN EXIT FUNCTION
  '
  ' find the parent
  FOR lngR = 1 TO UBOUND(ga_strConfig)
    lngObject = VAL(PARSE$(ga_strConfig(lngR),"",%Ref_ID))
    strObjectType = PARSE$(ga_strConfig(lngR),"",%Ref_Object)
    '
    IF lngObject = lngParent THEN
    ' parent found - now look for trigger value to
    ' see if this object is visible or not
      SELECT CASE strObjectType
        CASE "Tickbox"
          CONTROL GET CHECK hDlg, lngObject TO lngValue
          strSelection = FORMAT$(lngValue)
        CASE ELSE
          CONTROL GET TEXT hDlg,lngObject TO strSelection
      END SELECT
      '
        '
      IF strTrigger = strSelection THEN
      ' trigger matches current selection
      ' so make object/s visible
        CONTROL NORMALIZE hDlg,lngRefID
        '
        IF lngLabelControl > 0 THEN
        ' show any associated label control
          CONTROL NORMALIZE hDlg,lngLabelControl
        END IF
        '
      ELSE
        CONTROL HIDE hDlg,lngRefID
        '
        IF lngLabelControl > 0 THEN
        ' hide any associated label control
          CONTROL HIDE hDlg,lngLabelControl
        END IF
        '
      END IF
      '
...
' fill the form with data from the form data array
  ...
  FOR lngColumn = 1 TO PARSECOUNT(strHeaders,"")
    ...
    END IF
    '
    ' does this control have a parent?
    funHasParentControl(hDlg,lngConfigRow,strData)
    '
  NEXT lngColumn
  '
