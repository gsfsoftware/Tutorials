...
'------------------------------------------------------------------------------
#INCLUDE "..\Libraries\PB_BitmapGraphics.inc"
'------------------------------------------------------------------------------
'   ** Constants **
'------------------------------------------------------------------------------
...
FUNCTION funBuildGraphicsMap(hDlg AS DWORD, _
 ...
 ' draw a blank hex map
  ...
  lngY = 15
  '
  lngXOffSet = lngDrop + lngHexWidth - 2
  lngYOffSet = (lngHexWidth * 2) +10
  ...
  FOR lngR = 1 TO %MaxX
    FOR lngC = 1 TO %MaxY
      ...
      ' offset alternate columns
        lngYTemp = lngYTemp + lngHexWidth + 4
      END IF
      '
      ...
      '
      IF ISTRUE funGetBitmapSize(strBitmapMask, lngWidth,lngHeight) THEN
      '
      ' get the terrain type
        lngTerrain = g_lngTerrain(lngR,lngC,%Terrain)
        ' now store the centre of the hex
        g_lngTerrain(lngR,lngC,%X) = lngXStart + (lngWidth \ 2)
        g_lngTerrain(lngR,lngC,%Y) = lngYStart + (lngHeight \ 2)
        '
        strBitmap = EXE.PATH$ & "\Graphics\" & CHR$(lngTerrain) & ".bmp"
        funPasteBitmap(strBitmap,strBitmapMask, _
                       lngWidth,lngHeight, _
                       lngXstart, lngYstart, %IDC_GRAPHIC1)
          '
      END IF
      GRAPHIC BITMAP END
      '
      ...
      SELECT CASE lngC
        ...
        CASE %MaxY
          GRAPHIC COLOR %RGB_WHITE, -2
          GRAPHIC SET POS (lngPolygonX +6,lngPolygonY + lngHeight )
          GRAPHIC PRINT CHR$(lngXLetter + lngR);
          '
      END SELECT
      '
...
' look for Terrain = %Terrain.Grasslands for pickup
  ...
  LOCAL strBitmapMask AS STRING
  LOCAL strBitmap AS STRING
  LOCAL nFile&, lngWidth AS LONG, lngHeight AS LONG
  LOCAL lngXStart AS LONG
  LOCAL lngYStart AS LONG
  ...
  '
    ...
    '
    lngXStart = g_lngTerrain(lngR, lngC,%X) - (lngWidth \ 2)
    lngYStart = g_lngTerrain(lngR, lngC,%Y) - (lngHeight \ 2)
    '
    funPasteBitmap(strBitmap,strBitmapMask, _
                   lngWidth,lngHeight, _
                   lngXstart, lngYstart, %IDC_GRAPHIC1)
...
'TYPE udtBuilding
  ...
  FOR lngB = 1 TO %MaxBuildings
    ...
    IF ISTRUE funGetBitmapSize(strBitmapMask, lngWidth,lngHeight) THEN
    '
      lngXStart = g_lngTerrain(lngR, lngC,%X) - (lngWidth \ 2) + lngXOffset
      lngYStart = g_lngTerrain(lngR, lngC,%Y) - (lngHeight \ 2) + lngYOffset
      '
      funPasteBitmap(strBitmap,strBitmapMask, _
                   lngWidth,lngHeight, _
                   lngXstart, lngYstart, %IDC_GRAPHIC1)
