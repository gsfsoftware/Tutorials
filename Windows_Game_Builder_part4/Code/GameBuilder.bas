...
' position the rulers on the map
  ...
  FOR lngR = 1 TO %TotalRulers
    ...
    WHILE lngValid = %FALSE
      ...
      '
      IF MID$(a_strMap(lngY),lngX,1) = "1" THEN
      ' land area
...
#PBFORMS BEGIN DIALOG %IDD_dlgTerrainInfo->->
  ...
  DIALOG NEW hParent, "Terrain Inofrmation", 398, 192, 501, 350, %WS_POPUP OR _
  ...
  CONTROL ADD TEXTBOX, hDlg, %IDC_txtOwner, "", 60, 35, 45, 13, %WS_CHILD OR _
    ...
    %ES_READONLY, %WS_EX_CLIENTEDGE OR %WS_EX_LEFT OR %WS_EX_LTRREADING OR _
    %WS_EX_RIGHTSCROLLBAR
  CONTROL ADD LABEL,   hDlg, %IDC_lblOwnerDetails, "Owner Details", 20, 225, _
    55, 10
  CONTROL SET COLOR    hDlg, %IDC_lblOwnerDetails, %BLUE, -1
  CONTROL ADD TEXTBOX, hDlg, %IDC_txtOwnerDetails, "", 20, 236, 315, 95, _
    ...
    %WS_EX_RIGHTSCROLLBAR
  CONTROL ADD GRAPHIC, hDlg, %IDC_gra3DTerrain, "", 120, 5, 355, 220, _
    %WS_CHILD OR %WS_VISIBLE OR %WS_BORDER
  CONTROL ADD GRAPHIC, hDlg, %IDC_graSmallMap, "", 20, 95, 90, 85, %WS_CHILD _
...
#PBFORMS END DIALOG
  ...
  GRAPHIC ATTACH hDlg,%IDC_graSmallMap, REDRAW
  ...
  GRAPHIC REDRAW
  '
  GRAPHIC ATTACH hDlg,%IDC_gra3DTerrain, REDRAW
...
FUNCTION funDetermine_Terrain(BYREF a_lngHeight() AS LONG, _
  ...
  lngVOffset = 10      ' amount of vertical offset for land
  '
