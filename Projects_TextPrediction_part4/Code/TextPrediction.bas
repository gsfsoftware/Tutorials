...
$Metrics = "Metrics.txt"
'
%LineLength = 60        ' number of characters per line in text
'                         input box
        ...
        CASE %ID_Load
          ...
          ' load the training data arrays
            IF ISTRUE funLoadArrays(strError) THEN
              ...
              '
              ' enable the processingtoolbar button
              TOOLBAR SET STATE CB.HNDL, %MainToolbar, _
                                BYCMD %ID_PROCESS, %TBSTATE_ENABLED
              '
            ELSE
              MSGBOX strError, _
                     %MB_TASKMODAL OR %MB_ICONERROR, _
...
END FUNCTION
'
FUNCTION funRunProcessing(hDlg AS DWORD, _
                          lngTextField AS LONG) AS LONG
' get seeds from user and run the processing
  LOCAL strSeed AS STRING          ' seed word provided by user
  LOCAL strPrompt AS STRING        ' prompt on input box
  LOCAL strText AS STRING          ' text built up from data arrays
  LOCAL lngLoop AS LONG            ' loop number
  LOCAL lngLoopCount AS LONG       ' number of loop iterations
  LOCAL strWordSuggested AS STRING ' word that has been suggested
  LOCAL strLastTwoWords AS STRING  ' last two words in the text
  '
  ' arrays used to hold
  DIM a_strWord(1 TO %WordResult) AS STRING
  ' the results of the search
  DIM a_lngWordCount(1 TO %WordResult) AS LONG
  '
  lngLoopCount = 50
  '
  strPrompt = "Enter two words separated by a space"
  '
  strSeed = TRIM$(INPUTBOX$(strPrompt , "Text Seeding", "" )) & " "
  '
  ' now use seed to populate the text field
  ' first empty the field
  CONTROL SET TEXT hDlg,lngTextField, ""
  '
  IF ISTRUE funSuggestWord(strSeed, _
                           a_strWord(), _
                           a_lngWordCount()) THEN
    strText = strSeed & funPickWord(a_strWord(), _
                                    a_lngWordCount())
                                    '
  ELSE
  ' no words?
  ...
  FOR lngLoop = 1 TO lngLoopCount
    ' arrays used to hold
    REDIM a_strWord(1 TO %WordResult) AS STRING
    ' the results of the search
    REDIM a_lngWordCount(1 TO %WordResult) AS LONG
      ' get the last two words
    strLastTwoWords = PARSE$(strText," ",-2) & " " & _
                      PARSE$(strText," ",-1) & " "
  '
    IF ISTRUE funSuggestWord(strLastTwoWords, _
                             a_strWord(), _
                             a_lngWordCount()) THEN
      '
      strWordSuggested = funPickWord(a_strWord(), _
                                     a_lngWordCount())
      strText = strText & " " & strWordSuggested
      '
    END IF
    '
  NEXT lngLoop
  '
  CONTROL SET TEXT hDlg,lngTextField,strText
  funTrimText(hDlg,lngTextField)
  '
END FUNCTION
'
FUNCTION funPickWord(BYREF a_strWord() AS STRING, _
                     BYREF a_lngWordCount() AS LONG) AS STRING
' pick the work from the arrays
  LOCAL strWordSuggested AS STRING
  LOCAL lngTop AS LONG
  '
  lngTop = RND(1,5)
  WHILE strWordSuggested = ""
    IF a_strWord(lngTop) <> "" THEN
    ' only where word is not blank
      strWordSuggested = a_strWord(lngTop)
    END IF
    lngTop = RND(1,5)
  WEND
  '
  FUNCTION = TRIM$(strWordSuggested)
  '
END FUNCTION
...
' process a text change
  ...
  LOCAL strFirstWord AS STRING  ' first word
  LOCAL strSecondWord AS STRING ' second word
  LOCAL strPhrase AS STRING     ' combined words
  ...
  ' look for last two words
    ...
    ' build phrase to be searched for
      ...
      DIM a_lngWordCount(1 TO %WordResult) AS LONG
      '
      IF ISTRUE funSuggestWord(strPhrase, a_strWord(),a_lngWordCount() ) THEN
...
END FUNCTION
'
FUNCTION funSuggestWord(strPhrase AS STRING, _
