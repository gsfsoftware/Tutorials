  ...
  SELECT CASE wMsg
    ...
    ' get mouse co-ords and show popup dialog
      ...
      lngHex = funGetHex(pLocation.X, pLocation.Y, _
                         ...
                         '
      SELECT CASE lngHex
        CASE %MenuApp,%ReportApp
        ' show popup menu
          funBuildMenu(lngHex, lngX, lngY)
     END SELECT
     '
  END SELECT
  '
...
END FUNCTION
'
FUNCTION funBuildMenu(lngHex AS LONG, _
                      lngX AS LONG, _
                      lngY AS LONG) AS LONG
' show the popup menu
  LOCAL hPopup AS LONG
  LOCAL lngMenuPicked AS LONG
  '
  SELECT CASE lngHex
    CASE %MenuApp
    ' build popup menu
      ...
      ' add 2 menu options
      MENU ADD STRING, hPopup,"Menu A", 1, %MF_ENABLED
      MENU ADD STRING, hPopup,"Menu B", 2, %MF_ENABLED
      ' trigger on left mouse button
      MENU CONTEXT hPopup,lngX,lngY, %TPM_LEFTBUTTON TO lngMenuPicked
    '
    CASE %ReportApp
    ' build popup menu
      MENU NEW POPUP TO hPopup
      ' add 3 menu options
      MENU ADD STRING, hPopup,"Report A", 1, %MF_ENABLED
      MENU ADD STRING, hPopup,"Report B", 2, %MF_ENABLED
      MENU ADD STRING, hPopup,"Report C", 3, %MF_ENABLED
      ' trigger on left mouse button
      MENU CONTEXT hPopup,lngX,lngY, %TPM_LEFTBUTTON TO lngMenuPicked
    '
  END SELECT
  '
  ' check which menu was picked
  IF lngMenuPicked > 0 THEN
    MSGBOX "Picked = " & FORMAT$(lngMenuPicked)
  END IF
  '
END FUNCTION
