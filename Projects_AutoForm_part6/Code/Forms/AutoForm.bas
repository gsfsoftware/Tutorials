...
FUNCTION PBMAIN()
  ...
  ' set the current form number to the end of the array
    ...
    ' add a blank record to the end of the array
...
' first determine where to save the data
  ...
  ' so save to file
    FUNCTION = funArrayDump(strOutput,BYREF g_strFormData() )
    '
  ELSE
    ...
    ' control has been identified so populate it with data
      SELECT CASE strObject
        ...
        ' populate the text box
          CONTROL SET TEXT g_hDlg,lngControl, PARSE$(strData,"", _
                           lngColumn)
        CASE "Dropdownlist"
        ' populate the combo
          funSelectCombo(g_hDlg,lngControl, PARSE$(strData,"", _
                         lngColumn))
        CASE "Listbox"
        ' populate the listbox
          funSelectListbox(g_hDlg,lngControl, PARSE$(strData,"", _
                           lngColumn))
      END SELECT
      '
    END IF
