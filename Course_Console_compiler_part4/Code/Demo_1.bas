...
'
%Max_days = 900 ' max days to process
'
...
' run the main processing
  ...
  LOCAL strError AS STRING
  '
  ' output headers to file when saving row by row
  'funAppendToAFile(strOutputFile, _
  '                 $DQ & "Date" & $QCQ & "Day of Week" & $DQ, _
  '                 strError)
  ...
  'for lngD = 1 to 90
    ...
    '
    ' save to file row by row
    'funAppendToAFile(strOutputFile, _
    '               $DQ & a_strDays(lngD,%Date) & $QCQ & _
    '               a_strDays(lngD,%Day) & $DQ, _
    '               strError)
    'if strError <> "" then
    '  exit loop
    'end if
   ...
   WEND
   '
   ' test for errors when saving row by row
   'if strError = "" then
   '  function = %TRUE
   'end if
