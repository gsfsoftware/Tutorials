...
%LV_Columns = 3                  ' number of columns in the staff listview grid
'------------------------------------------------------------------------------
'   ** Declarations **
'------------------------------------------------------------------------------
        ...
        CASE %IDC_Submit
          ...
          ' so validate input
            ...
            ' and confirm to user
              ...
              LISTVIEW SET TEXT CB.HNDL, %IDC_lvStaffDataGrid, _
                 lngRow, 2, strDepartment
              LISTVIEW SET TEXT CB.HNDL, %IDC_lvStaffDataGrid, _
                 lngRow, 3, strStartDate
                 '
              ' save the listview
...
' clear all the form entries ready for more input
  ...
  strDate = DATE$
  funSetaDate(hDlg,%IDC_datStartDate,strDate)
  '
END FUNCTION
'
FUNCTION funSetaDate(hDlg AS DWORD, lngDateControl AS LONG, _
                     strDate AS STRING) AS LONG
' set a date control to the date passed - mm/dd/yyy format assumed
...
#PBFORMS END DIALOG
  ...
  PREFIX "LISTVIEW INSERT COLUMN hDlg, "
    %IDC_lvStaffDataGrid, 1, "Name", 50,%lv.Center
    %IDC_lvStaffDataGrid, 2, "Department", 50,%lv.Center
    %IDC_lvStaffDataGrid, 3, "Start Date", 50,%lv.Center
  END PREFIX
  '
  ' your code goes here
  ...
  ' only if the file exists
    ...
    ' populate the list view
      ...
      LISTVIEW INSERT ITEM hDlg, %IDC_lvStaffDataGrid, _
                                 lngRow, 0, a_strStaffData(lngRow,1)
      ' populate the remaining columns
      LISTVIEW SET TEXT hDlg, %IDC_lvStaffDataGrid, _
                              lngRow, 2, a_strStaffData(lngRow,2)
      LISTVIEW SET TEXT hDlg, %IDC_lvStaffDataGrid, _
                              lngRow, 3, a_strStaffData(lngRow,3)
     NEXT lngRow
    '
  END IF
  '
