...
'
#IF NOT %DEF(%Monitor)
' but only if this is not the monitoring application
#INCLUDE "BlockChainTransactions.inc" ' routines for BlockChain transactions
#ENDIF
'
...
'
$BlockChainFile = "DemoBlockChainFile.txt" ' name of saved Blockchain
$ClosingBalance = "ClosingBalance.txt"     ' number of units left
'
...
' the main PB function that is the first to be executed
  ...
  '
  '
  ' prep the blockchain collection
  #IF NOT %DEF(%Monitor)
  ' but only if this is not the monitoring application
  ' define BlockChain class
  LET BlockChain = CLASS "PowerCollection"
  '
  ' load existing blockchain from file?
  IF ISTRUE ISFILE(EXE.PATH$ & $BlockChainFile) THEN
    funLoadBlockChainFromFile(EXE.PATH$ & $BlockChainFile)
  ELSE
     IF ISTRUE funInitialiseBlockChain() THEN
    ' create Genesis block
    ELSE
      funLog("unable to prep Blockchain storage")
    END IF
    '
  END IF
  #ENDIF
  ...
  ' but only if this is not the monitoring appliction
    ...
    ' save the current blockchain to disk
    funSaveBlockChainToFile(EXE.PATH$ & $BlockChainFile)
    ' save closing balance of transactions
    funSaveClosingBalance(EXE.PATH$ & $ClosingBalance)
  #ENDIF
  '
...
'
  ...
  LOCAL strTimeStamp AS STRING    ' to hold timestamp of the transaction
  '
  '
    ...
    DO UNTIL ISTRUE g_lngEnding
      ...
      ' only compile these in if not monitoring
        ...
        ' we have a node to transact with
          strTimeStamp = funGetTimeStamp
          funRaiseTransaction(strTransaction,strTimeStamp)
        END IF
        '
      #ENDIF
      '
            ...
            ' incoming transaction
              ...
              ' only handle if not monitoring
                ...
                ' pick up number of assets
                  ...
                  strTimeStamp = PARSE$(strMessage,"|",5)
                  funConfirmTransaction(strTransaction, strTimeStamp)
                  '
                  ' now add to blockchain
                  strMessage = funFormatTransactionData_transaction(strMessage)
                  '
                  IF ISTRUE funAddToBlockchain(strMessage,strTimeStamp) THEN
                    funAppendToFile("Audit.txt","My purchase " & strMessage)
                  ELSE
                    funAppendToFile("Audit.txt","My failed purchase " & strMessage)
                  END IF
                  '
                END IF
                '
            ...
            ' incoming confirmation
              ...
              ' only handle if not monitoring
                ...
                ' confirmation is for this node
                  ...
                  g_lngAssets = g_lngAssets - lngAssetsReceived
                  '
                  ' *** Transaction has been confirmed so add to blockchain
                  strTimeStamp = PARSE$(strMessage,"|", 5) ' pick up time stamp
                  ' reformat the message for the block chain
                  strMessage = funFormatTransactionData_confirm(strMessage)
                  '
                  IF ISTRUE funAddToBlockchain(strMessage, strTimeStamp) THEN
                    funAppendToFile("Audit.txt","My sale " & strMessage)
                  ELSE
                    funAppendToFile("Audit.txt","My Failed Add " & strMessage)
                  END IF
                '
                ELSE
                ' another nodes confirmation
                  strTimeStamp = PARSE$(strMessage,"|", 5)
                  strMessage = funFormatTransactionData_confirm(strMessage)
                  '
                  IF ISTRUE funAddToBlockchain(strMessage,strTimeStamp) THEN
                    funAppendToFile("Audit.txt","Other sale " & strMessage)
                  ELSE
                    funAppendToFile("Audit.txt","Other Failed Add " & strMessage)
                  END IF
                '
                END IF
                '
...
END FUNCTION
'
FUNCTION funFormatTransactionData_confirm(strMessage AS STRING) AS STRING
  ' reformat the data for storage in block chain
  ' using confirmation message
  LOCAL strData AS STRING
  strData = PARSE$(strMessage,"|",3) & "-" & _
            "Sells - " & _
            PARSE$(strMessage,"|",4) & "- TO -" & _
            PARSE$(strMessage,"|",1)
  FUNCTION = strData
END FUNCTION
'
...
END FUNCTION
'
FUNCTION funFormatTransactionData_transaction(strMessage AS STRING) _
                                              AS STRING
  ' reformat the data for storage in block chain
  ' using transaction message
  LOCAL strData AS STRING
  strData = PARSE$(strMessage,"|",1) & "-" & _
            "Sells - " & _
            PARSE$(strMessage,"|",4) & "- TO -" & _
            PARSE$(strMessage,"|",3)
  FUNCTION = strData
...
END FUNCTION
'
FUNCTION funSaveClosingBalance(strFile AS STRING) AS LONG
' save the closing balance for this node
  LOCAL lngFile AS LONG
  '
  lngFile = FREEFILE
  OPEN strFile FOR OUTPUT AS #lngFile
  PRINT #lngFile,FORMAT$(g_lngAssets)
  CLOSE #lngFile
  '
END FUNCTION
