...
'
  ...
  LOCAL strCellContents AS STRING   ' contents of the cell
  '
  ...
  ' look through each column
    ...
    ' of that column
       ...
       '
       ELSE
       ' is this text or image in the table cell?
       ' get the cell contents
         strCellContents = a_strTable(lngRow,lngColumn)
         IF INSTR(strCellContents,"<img src") > 0 THEN
           ' it's an image
              ' store the widest column
              a_lngTableColumns(lngColumn) = _
                               MAX(a_lngTableColumns(lngColumn), _
                               funGetWidthOfImage(strCellContents, _
                                                  lngIsWebPage, _
                                                  lngTabSelected, _
                                                  hDlg) )
              ' add on a small right hand margin
              a_lngTableColumns(lngColumn) += lng_imgW

         ELSE
         ' its just text
         ' first store number of characters in array
           a_lngTableColumns(lngColumn) = _
                             MAX(LEN(strCellContents), _
                             LEN(a_strTable(lngRow -1,lngColumn)))
                               '
         ' store number of characters * character width
...
END FUNCTION
'
FUNCTION funGetWidthOfImage(strCellContents AS STRING, _
                            lngIsWebPage AS LONG, _
                            lngTabSelected AS LONG, _
                            hDlg AS DWORD) AS LONG
' return the width of an image
  ...
  '
  lngStartPosition = INSTR(strCellContents,"src=") + LEN("src=")
  strTemp = MID$(strCellContents,lngStartPosition)
